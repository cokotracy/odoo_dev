...
class HrAppraisalGoal(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', string="Owner",
        default=lambda self: self.env.user.employee_id, required=False) #Changed Required
    manager_id = fields.Many2one('hr.employee', string="Challenged By", required=False) #Changed Required
    progression = fields.Selection(selection=[
        ...
        ('75', '75 %'),
        ('100', '100 %')
    ], string="Progression", default="0", required=False) #Changed Required
