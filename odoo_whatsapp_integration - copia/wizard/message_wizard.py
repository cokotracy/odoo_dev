...
class MessageError(models.TransientModel):
    ...
    def get_message(self):
        ...
        return False
    name = fields.Text(string="Mensaje", readonly=True, default=get_message)

class SendMessage(models.TransientModel):
    _name = 'whatsapp.wizard'

    user_id = fields.Many2one('res.partner', string="Destinatario", default=lambda self: self.env[self._context.get('active_model')].browse(self.env.context.get('active_ids')).partner_id)
